...
const CountdownOverlay = ({ seconds, onFinished }: {
    ...
    return (
        <div className={`absolute top-0 left-0 bottom-0 right-0 flex items-center justify-center bg-[rgba(0,0,0,0.5)] w-full z-10 ${counter < 1 ? 'fade-out' : ''}`}>
            <div className='flex items-center justify-center gap-2 p-4 border-4 border-background rounded-full relative shadow-lg w-20 h-20 z-20
                overflow-hidden'>
                <span className='text-xl font-bold border-background z-20 bg-[#cacaca] rounded-full w-[16px] h-[16px] px-[10px] py-[10px] flex items-center justify-center' style={{
                    lineHeight: '16px'
                }}>{counter}</span>
